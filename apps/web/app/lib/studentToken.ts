...
function getSecret() {
  return encoder.encode(process.env.STUDENT_JWT_SECRET ?? "dev-insecure-secret")
}
...
export async function verifyStudentToken(token: string) {
  ...
  try {
    ...
    const payload = JSON.parse(decoder.decode(payloadBytes)) as Partial<StudentPayload>
...
export async function signStudentToken(studentId: string, ttlSeconds: number) {
