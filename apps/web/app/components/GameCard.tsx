    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-base font-extrabold tracking-tight text-zinc-950 dark:text-white">
              {game.title}
            </div>
            <div className="mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">
              {game.blurb ?? "点击开始，赢取星空币"}
            </div>
          </div>
        </div>
        ...
        {game.locked ? (
          <div className="flex flex-col items-end gap-1">
            <div className="rounded-2xl border border-black/10 bg-white/70 px-3 py-2 text-xs font-extrabold text-zinc-700 dark:border-white/10 dark:bg-white/5 dark:text-zinc-200">
              🔒 需要{game.requiresLabel ?? "更高进度"}
            </div>
          ...
          <Link
            href={`/games/${game.id}`}
            className="relative inline-flex h-10 items-center justify-center rounded-2xl bg-zinc-950 px-4 text-sm font-extrabold text-white shadow-sm transition hover:bg-zinc-800 dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-200"
          >
            开始玩
          </Link>
        )}
