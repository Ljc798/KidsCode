...
const secret = () => process.env.STUDENT_JWT_SECRET ?? "dev-insecure-secret"
...
export function signStudentToken(studentId: string, ttlSeconds: number) {
...
export function verifyStudentToken(token: string) {
  ...
  try {
    ...
    return { studentId: payload.sub as string }
  } catch {
    return null
  }
}
...
export function getTokenFromRequest(req: {
  ...
  return cookie
    .split(";")
    .map(s => s.trim())
    .find(s => s.startsWith("kidscode_student="))
    ?.slice("kidscode_student=".length)
}
